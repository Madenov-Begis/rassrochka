...
## Задача: Привязка рассрочки к менеджеру магазина
- **Статус**: В процессе
- **Описание**: Каждая рассрочка должна быть привязана к конкретному менеджеру (store_manager) магазина. Менеджер выбирается при создании и может быть изменён при редактировании. На фронте реализовать отображение и фильтрацию по менеджеру.
- **Шаги выполнения**:
  - [ ] Добавить поле managerId в модель Installment (Prisma)
  - [ ] Провести миграцию БД
  - [ ] Обновить DTO, сервисы и контроллеры backend
  - [ ] Реализовать выбор и смену менеджера на фронте
  - [ ] Отображать менеджера в списке и деталях рассрочки
  - [ ] Реализовать фильтрацию по менеджеру в API и на фронте
  - [ ] Обновить документацию (project.md, changelog.md)
- **Зависимости**: users (store_manager), installments 