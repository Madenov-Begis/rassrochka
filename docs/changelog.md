...
## [2024-07-16] - Упрощена логика досрочного погашения рассрочки
### Изменено
- Досрочное погашение теперь возможно в любой момент при отсутствии просрочек, независимо от количества оставшихся месяцев
- Все pending-платежи отменяются, создаётся один платёж с type = "early_payoff", status = "paid", сумма = остаток основного долга (без процентов)
- Рассрочка переводится в статус "early_payoff", totalAmount корректируется
- На фронте платёж выделяется и сопровождается пояснением: "Досрочное погашение, проценты не начислены"

### Исправлено
- Исключена ошибка, когда досрочное погашение было возможно только при одном pending-платеже 

## [2024-07-17] - Привязка рассрочки к менеджеру магазина
### Добавлено
- В модель Installment (Prisma) добавлено поле managerId (User, роль store_manager)
- Миграция БД для поддержки связи рассрочки с менеджером
- Обновлены DTO, сервисы и контроллеры backend для выбора/смены менеджера
- На фронте реализован выбор менеджера при создании/редактировании рассрочки
- Отображение менеджера в списке и деталях рассрочки
- Фильтрация рассрочек по менеджеру в API и на фронте
- Обновлена документация (project.md, tasktracker.md) 