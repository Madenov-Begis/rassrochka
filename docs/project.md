...
## Привязка рассрочки к менеджеру магазина

С июля 2024 каждая рассрочка (installment) обязательно привязывается к конкретному менеджеру магазина (store_manager):
- Менеджер выбирается из списка менеджеров текущего магазина при создании рассрочки (обязательное поле).
- Менеджера можно изменить при редактировании рассрочки.
- В списке и деталях рассрочки на фронте отображается ФИО менеджера.
- В API реализована фильтрация рассрочек по менеджеру (query-параметр managerId).
- Менеджер видит все рассрочки своего магазина (фильтрация по менеджеру — опциональна для статистики).
- Валидация: выбранный менеджер должен принадлежать текущему магазину.

### Архитектурные изменения
- В модель Installment (Prisma) добавлено поле managerId (User, роль store_manager).
- Обновлены DTO, сервисы и контроллеры backend для поддержки выбора и смены менеджера.
- На фронте реализован выбор менеджера при создании/редактировании, отображение и фильтрация.
