    ...
    return (
      ...
      >
        ...
        <div className="space-y-1">
          {payments?.data?.slice(0, 2).map((payment: any) => (
            <div
              key={payment.id}
              className="text-xs p-1 bg-yellow-100 text-yellow-800 rounded truncate"
              title={`${payment.installment.customer.firstName} ${payment.installment.customer.lastName} - ${Number(payment.amount).toLocaleString('ru-RU', { maximumFractionDigits: 0 })} UZS`}
            >
              {Number(payment.amount).toLocaleString('ru-RU', { maximumFractionDigits: 0 })}
            </div>
          ))}
          {payments && payments.length > 2 && <div className="text-xs text-gray-500">+{payments.length - 2} еще</div>}
        </div>
      </div>
    )
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">Визуализация предстоящих и просроченных платежей</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Предстоящие платежи</CardTitle>
              <Clock className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">{upcomingPayments?.data?.length || 0}</div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Сумма на неделю</CardTitle>
              <Calendar className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {upcomingPayments
                  ?.data?.reduce((sum: number, payment: any) => sum + Number(payment.amount), 0)
                  .toLocaleString() || 0}
              </div>
              <p className="text-xs text-muted-foreground">Ожидаемые поступления</p>
            </CardContent>
          </Card>
        </div>
          ...
          <CardContent>
            <div className="grid grid-cols-7 gap-0 border border-gray-200 rounded-lg overflow-hidden">
              ...
              {["Вс", "Пн", "Вт", "Ср", "Чт", "Пт", "Сб"].map((day) => (
                <div key={day} className="p-3 bg-gray-50 border-b border-gray-200 text-center font-medium text-sm">
                  {day}
                </div>
              ))}
              ...
              {Array.from({ length: firstDay }).map((_, index) => (
                <div key={`empty-${index}`} className="min-h-[80px] bg-gray-50 border-b border-gray-200"></div>
              ))}
                ...
                {overduePayments?.data?.map((payment: any) => (
                  <div key={payment.id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg">
                    <div>
                      ...
                      <div className="text-sm text-gray-600">{payment.installment.customer.phone}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-red-600">{Number(payment.amount).toLocaleString('ru-RU', { maximumFractionDigits: 0 })}</div>
