  ...
  return (
    <DashboardLayout>
      <div className="max-w-2xl mx-auto py-8 space-y-8">
        <Card>
          <CardHeader>
            <CardTitle>Глобальный поиск по паспорту</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSearch} className="flex gap-2 items-end">
              ...
              <Button type="submit" className="h-10"><Search className="w-4 h-4 mr-2" />Искать</Button>
            </form>
          </CardContent>
        </Card>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div>
                      ...
                      {block.installments?.length === 0 ? (
                        <div className="text-muted-foreground">Нет рассрочек в этом магазине</div>
                      ) : (
                        <Table>
                          <TableHeader>
                            <TableRow>
                          ...
                          <TableBody>
                            {block.installments.map((i) => (
                              <TableRow key={i.id} className={i.status === "overdue" ? "bg-red-50" : ""}>
                                <TableCell>{i.productName}</TableCell>
                                <TableCell>{Number(i.totalAmount).toLocaleString('ru-RU', { maximumFractionDigits: 0 })} UZS</TableCell>
                                <TableCell><Badge>{i.status === "overdue" ? "Просрочка" : i.status === "active" ? "Активна" : "Завершена"}</Badge></TableCell>
