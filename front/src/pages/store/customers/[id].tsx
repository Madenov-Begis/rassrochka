...
export default function CustomerDetailPage() {
  ...
  const blacklistMutation = useMutation({
    ...
    onSuccess: (_, isBlacklisted) => {
      ...
      toast.success(
        isBlacklisted ? 'Клиент заблокирован' : 'Клиент разблокирован',
      );
    },
  });
            ...
            <Card className="text-xs sm:text-sm">
              ...
              <CardContent>
                ...
                <div className="border-t pt-4">
                  ...
                  <p className="font-medium">
                    {customer.data?.passportSeries}{' '}
                    {customer.data?.passportNumber}
                  </p>
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <Button
                  ...
                  onClick={() =>
                    navigate(
                      `/store/installments/create?customerId=${customer.data?.id}`,
                    )
                  }
                >
                ...
                <Button
                  variant={
                    customer.data?.isBlacklisted ? 'outline' : 'destructive'
                  }
                  className="w-full"
                ...
                >
                  <DialogContent>
                    ...
                    <Alert
                      variant={
                        customer.data?.isBlacklisted ? 'default' : 'destructive'
                      }
                    >
                      <AlertDescription>
                        {customer.data?.isBlacklisted
                ...
                {overdueInstallments > 0 && (
                  <Alert>
                    ...
                    <AlertDescription>
                      У клиента есть {overdueInstallments} просроченных
                      рассрочек. Рекомендуется связаться с клиентом.
                    </AlertDescription>
                  </Alert>
                )}
              </CardContent>
