...
export function CreateInstallmentForm({ onSuccess }: CreateInstallmentFormProps) {
  ...
  const installmentSchema = z.object({
    ...
    interestRate: z.number().min(0, "Ставка не может быть отрицательной").max(100, "Ставка не может быть больше 100%"),
    months: z.number().min(1, "Срок должен быть не менее 1 месяца").max(120, "Срок не может превышать 120 месяцев"),
    customerId: z.number(),
  ...
  const handleCalculate = async () => {
    ...
    if (valid) {
      ...
      if (
        typeof values.productPrice === 'number' && !isNaN(values.productPrice) &&
        typeof values.downPayment === 'number' && !isNaN(values.downPayment) &&
        typeof values.interestRate === 'number' && !isNaN(values.interestRate) &&
        typeof values.months === 'number' && !isNaN(values.months)
      ) {
        const base = values.productPrice - values.downPayment;
        const totalInterest = base * values.interestRate * values.months / 100;
        ...
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Select onValueChange={(value) => setValue("customerId", Number(value))}>
                <SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <InputAmount
                ...
                onChange={val => {
                  setValue("productPrice", val ? Number(val) : 0, { shouldValidate: true })
                }}
            ...
            <div className="space-y-2">
              ...
              <InputAmount
                ...
                onChange={val => setValue("downPayment", val ? Number(val) : 0, { shouldValidate: true })}
            ...
            <div className="space-y-2">
              <Label htmlFor="interestRate">Процентная ставка в месяц (%) *</Label>
              <Input
                id="interestRate"
                type="number"
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              <Button type="button" onClick={handleCalculate} disabled={createMutation.isPending}>
                Рассчитать
              </Button>
            </div>
          ...
          {showCalculation && calculation ? (
            <div className="space-y-3">
              <div className="flex justify-between">
                ...
                <span className="font-medium">{calculation.base.toLocaleString()} UZS</span>
              </div>
              <div className="flex justify-between">
                <span>Переплата:</span>
                <span className="font-medium text-orange-600">{calculation.totalInterest.toLocaleString()} UZS</span>
              </div>
              <div className="flex justify-between">
                <span>Общая сумма:</span>
                <span className="font-medium">{calculation.totalAmount.toLocaleString()} UZS</span>
              </div>
              <div className="flex justify-between text-lg font-bold border-t pt-2">
                <span>Ежемесячный платеж:</span>
                <span className="text-blue-600">{calculation.monthlyPayment.toLocaleString()} UZS</span>
              </div>
            </div>
          ) : (
            <div className="text-gray-400 text-center pt-12">Заполните форму и нажмите "Рассчитать"</div>
          )}
        </CardContent>
      </Card>
    </div>
