...
import { LayoutDashboard, Store, Users, CreditCard, UserCheck, BarChart3, ShoppingBag, FileText, Ban } from "lucide-react"
    ...
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
      ...
      <div className="flex items-center gap-2 px-6 py-4 border-b border-gray-200">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">{user?.role === "admin" ? "Админ-панель" : "Панель магазина"}</p>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 py-4 space-y-2">
        {navItems.map((item) => {
          const isActive =
            ...
            (item.href !== "/admin" && item.href !== "/store" && location.pathname.startsWith(item.href))
          ...
          <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div
              className={cn(
                ...
                user.store.status === "payment_overdue" && "bg-yellow-100 text-yellow-800",
